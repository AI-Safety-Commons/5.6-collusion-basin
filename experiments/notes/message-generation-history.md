# Message-generation setup history

> Archived setup notes. Historical commands and workspace observations below are retained for context. Use the [current guide](../../docs/message-generation.md) for setup; the referenced run artifacts are not included in a checkout.
...
In the original experiment workspace, the venv, pinned archive, imported corpus, inspected prompts, and mock run had already been created. These artifacts are not included in the repository. For a new checkout, follow the current setup guide; import and prepare deliberately refuse to overwrite. `fetch` reuses only a cache with the expected checksum. Downloads and outputs are gitignored. The committed source lock allows another checkout to retrieve the same archive; upstream availability is still required. No archive license is asserted by this project.
